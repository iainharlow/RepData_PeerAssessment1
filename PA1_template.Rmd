...
The interval with the greatest number of daily steps is between `r max_interval_start` and `r max_interval_end`, with a mean step rate of `r max_steprate_nice` steps per minute. Again, this doesn't print out the values when I look at the MD file in github, so...
...
The interval between `r maxtotal_interval_start` and `r maxtotal_interval_end` included the greatest total number of steps: `r max_steptotal`. Once again, to deal with the failure to render as expected in .MD format...
...
The total number of missing step values is `r num_NAs`. Or, printed directly:
```{r}
print(num_NAs)
```
